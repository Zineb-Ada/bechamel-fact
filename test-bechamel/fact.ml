...
let benchmark () =
  ...
  in
  let test_equal =
    Test.make_grouped ~name:"equal" ~fmt:"%s %s" [ test_equal0; test_equal1 ]
  in
  let test_compare =
  ...
  in
  let test_exists =
    Test.make_grouped ~name:"exists" ~fmt:"%s %s" [ test_exists0; test_exists1 ]
  in
  let test_find =
    Test.make_grouped ~name:"find" ~fmt:"%s %s" [ test_find0; test_find1 ]
  in
  let raw_results =
